...
class Dialog {
    ...
    show() {
        ...
        return new Promise(resolve => {
            // 优化对话框动画
            setTimeout(() => {
                const paperElement = this.$el.querySelector('x-paper');
                if (paperElement) {
                    paperElement.classList.add('dialog-entered');
                }
                resolve();
            }, 50);
        });
...
class ReceiveDialog extends Dialog {
    ...
    _nextFile(nextFile) {
        console.log('📁 ReceiveDialog: _nextFile called with:', nextFile);
        if (nextFile) this._filesQueue.push(nextFile);
        if (this._busy) return;
        this._busy = true;
        const file = this._filesQueue.shift();
        console.log('📁 ReceiveDialog: 准备显示文件:', file);
    ...
    _displayFile(file) {
        console.log('📁 ReceiveDialog: _displayFile called with:', file);
        const $a = this.$el.querySelector('#download');
        if (!$a) {
            console.error('❌ ReceiveDialog: 找不到 #download 元素');
            return;
        }
        if (!file.blob) {
        ...
        console.log('📁 ReceiveDialog: 显示下载对话框');
        this.show();
...
class SendTextDialog extends Dialog {
    ...
    _send(e) {
        e.preventDefault();
        console.log('📤 Sending text to recipient:', this._recipient);
